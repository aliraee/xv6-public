...
scheduler(void)
{
  ...
  for(;;){
    ...
    struct proc *highest_p=0; //highest priority
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for (tmp = ptable.proc; tmp < &ptable.proc[NPROC]; tmp++)
      {
        ...
        if (highest_p->priority > tmp->priority){
          highest_p = tmp;
        }
...
waitx(int *wtime ,int *rtime){
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        // show proccess times
        cprintf("pid=%d,StartTime=%d,EndTime=%d,RTime=%d,IOTime=%d,Ticks=%d\n",
                p->pid,p->stime,p->etime,p->rtime,p->iotime,ticks);
        *rtime=p->rtime;
        *wtime=p->etime - (p->stime + p->iotime + p->rtime); //calculate waiting time
        pid = p->pid; //helped from wait()
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir);
        p->pid = 0;
